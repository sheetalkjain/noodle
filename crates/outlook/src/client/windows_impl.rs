// Windows-specific Outlook client using COM APIs
...
impl InnerClient {
    ...
    fn map_item_to_email(&self, item: &ComDispatch) -> Result<Email> {
        ...
        // Get To recipients
        let to_var = item.get_property("To")?;
        let to = BSTR::try_from(&to_var)
            .map(|s| s.to_string())
            .unwrap_or_default();

        // Get CC recipients
        let cc_var = item.get_property("CC")?;
        let cc = BSTR::try_from(&cc_var)
            .map(|s| s.to_string())
            .ok()
            .filter(|s| !s.is_empty());
